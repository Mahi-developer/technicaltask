    ...
    async def get(self, request, job_id):
        ...
        try:
            ...
            invalid_resp = form_json_response(
                "failed", status_code=400, error_message="Invalid Job id, Provide a valid Job Id to get results."
            )
            if not job_id:
                return invalid_resp
...
class Movies(View):
    ...
    def form_movies_result(movies, directors, total_results: str):
        ...
        response = {
            ...
            "total_page": math.ceil(total_results / curr_config.OMDB_RESULT_PER_PAGE)
        }
        for movie in movies:
            results.append({
                "title": movie["Title"],
    ...
    async def get(self, request):
        ...
        try:
            ...
            cached_resp = await redis.aget(cache_key)
            if cached_resp:
                logger.info("Response available in cache, returning cached response.")
                return form_json_response("success", 200, addl_resp=json.loads(cached_resp))
 
                
            if not search_param:
                logger.info("No search params given, returning empty result")
                resp = self.form_movies_result(movies=[], directors={}, total_results="0")
            ...
            if status_code != 200 or not movies:
                ...
                return form_json_response(
                    "failed", status_code or 400, addl_resp=response,
            ...
            resp = self.form_movies_result(movies=movies, directors=directors, total_results=response["totalResults"])
            # set response to redis cache
            await redis.aset(cache_key, json.dumps(resp), ttl=curr_config.MOVIES_CACHE_TTL)
            return form_json_response("success", 200, addl_resp=resp)
        except Exception:
